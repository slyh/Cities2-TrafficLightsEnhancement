...
using SystemUpdatePhase = Game.SystemUpdatePhase;
using EndFrameBarrier = Game.EndFrameBarrier;

namespace C2VM.TrafficLightsEnhancement.Systems.TrafficLightSystem;

[CompilerGenerated]
public partial class PatchedTrafficLightSystem : Game.GameSystemBase
{
    [BurstCompile]
    private struct UpdateTrafficLightsJob : IJobChunk
    {
        ...
        private int GetNextSignalGroup(NativeList<Entity> laneSignals, TrafficLights trafficLights, bool preferChange, out bool canExtend, ref CustomTrafficLights customTrafficLights)
        {
            ...
            for (int i = 0; i < laneSignals.Length; i++)
            {
                ...
                if (m_ExtraLaneSignalData.HasComponent(entity3))
                {
                    extraLaneSignal = m_ExtraLaneSignalData[entity3];
                }
        ...
        private void UpdateLaneSignals(NativeList<Entity> laneSignals, TrafficLights trafficLights)
        {
            for (int i = 0; i < laneSignals.Length; i++)
            {
                ...
                ExtraLaneSignal extraLaneSignal = new ExtraLaneSignal();
                if (m_ExtraLaneSignalData.HasComponent(entity))
                {
                    extraLaneSignal = m_ExtraLaneSignalData[entity];
                }
    ...
    protected override void OnUpdate()
    {
        ...
        JobHandle dependency = JobChunkExtensions.ScheduleParallel(new UpdateTrafficLightsJob
        {
            ...
            m_CustomTrafficLightsType = InternalCompilerInterface.GetComponentTypeHandle(ref __TypeHandle.__TLE_CustomTrafficLights_RO_ComponentTypeHandle, ref base.CheckedStateRef)
